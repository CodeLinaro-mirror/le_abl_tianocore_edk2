...
static EFI_STATUS map_bootimg(const SNAP_BOOT_SELECTION_t *boot_select, Slot *BootableSlot, const CHAR8 *snap_kernel);
...
static EFI_STATUS load_environment_from_part(const CHAR8 *partName, SNAP_BOOT_SELECTION_t **BootSelect);
static EFI_STATUS save_environment_to_part(SNAP_BOOT_SELECTION_t *boot_select, const CHAR8 *partName);
static EFI_STATUS SaveImageToPartition (VOID *ImageBuffer, UINT32 ImageSize, CHAR16 *Pname);

EFI_STATUS snap_get_target_boot_params(Slot *BootableSlot, CHAR8 **cmdline)
{
    EFI_STATUS Status = EFI_SUCCESS;
    CHAR8 *snap_core, *snap_kernel;
    CHAR8 *reboot_reason = NULL;
    ...
    if ( !AsciiStrnCmp(boot_select->reboot_reason, FACTORY_RESET, AsciiStrLen(FACTORY_RESET))) {
      reboot_reason = boot_select->reboot_reason;
    }
    Status = map_bootimg(boot_select, BootableSlot, snap_kernel);
    if (reboot_reason)
        AsciiSPrint(cmdline_buf, sizeof(cmdline_buf), " snap_core=%a snap_kernel=%a %a", snap_core, snap_kernel, reboot_reason);
    else
        AsciiSPrint(cmdline_buf, sizeof(cmdline_buf), " snap_core=%a snap_kernel=%a", snap_core, snap_kernel);
    *cmdline = cmdline_buf;
    cleanup:
        // if (boot_select != NULL) {
...
EFI_STATUS map_bootimg(const SNAP_BOOT_SELECTION_t *boot_select, Slot *BootableSlot, const CHAR8 *snap_kernel)
{
    for (size_t n = 0; n < SNAP_BOOTIMG_PART_NUM; ++n) {
        if (!AsciiStrnCmp(boot_select->bootimg_matrix[n][1], snap_kernel, SNAP_NAME_MAX_LEN)) {
            if (AsciiStrLen(boot_select->bootimg_matrix[n][0])) {
                // we need only slot part of partition table ( '_a' or '_b')
                AsciiStrToUnicodeStr(boot_select->bootimg_matrix[n][0] + (AsciiStrLen(boot_select->bootimg_matrix[n][0]) - 2), BootableSlot->Suffix);
                return EFI_SUCCESS;
            }
        }
    }
...
static EFI_STATUS load_environment_from_part(const CHAR8 *partName, SNAP_BOOT_SELECTION_t **BootSelect)
{
    ...
    if ( boot_select->version != SNAP_BOOTSELECT_VERSION || boot_select->signature != SNAP_BOOTSELECT_SIGNATURE )
    {
        DEBUG ((EFI_D_ERROR, "snap: load_environment(%s): ERROR version/signature broken [0x%X] vs [0x%X], [0x%X] vs [0x%X]!\n",
            Pname, boot_select->version, SNAP_BOOTSELECT_VERSION, boot_select->signature, SNAP_BOOTSELECT_SIGNATURE));
