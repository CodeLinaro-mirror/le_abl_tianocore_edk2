...
ifeq ($(BUILD_NATIVE_AARCH64),true)
	...
	export LOAD_KM_AND_SET_ROT := true
	...
	export STORE_GPT_ATTR_IN_RECOVERYINFO := 1
	export GET_VIP_BID_INFO := 1
	export ETH_DT_PATCH_NEEDED := 1
	export TARGET_BOARD_TYPE_AUTO := 1
	export SOC_OP_MODE_IN_CMDLINE := 1
	export FLASHING_LOCK_WITHOUT_MISC_PART := 1
endif
...
ifeq ($(STORE_GPT_ATTR_IN_RECOVERYINFO), 1)
        STORE_GPT_ATTR_IN_RECOVERYINFO := 1
else
        STORE_GPT_ATTR_IN_RECOVERYINFO := 0
endif
...
ifeq ($(SCMI_UPDATES_NEEDED), 1)
	SCMI_UPDATES_NEEDED := 1
else
	SCMI_UPDATES_NEEDED := 0
endif

ifeq ($(GET_VIP_BID_INFO), 1)
	GET_VIP_BID_INFO := 1
else
	UPDATE_ETH_BID := 0
endif

ifeq ($(ETH_DT_PATCH_NEEDED), 1)
	ETH_DT_PATCH_NEEDED := 1
else
	ETH_DT_PATCH_NEEDED := 0
endif
...
ifeq ($(EMMC_MULTI_LUN_SUPPORT), 1)
        EMMC_MULTI_LUN_SUPPORT := 1
else
        EMMC_MULTI_LUN_SUPPORT := 0
endif

ifeq ($(SOC_OP_MODE_IN_CMDLINE), 1)
	SOC_OP_MODE_IN_CMDLINE := 1
else
	SOC_OP_MODE_IN_CMDLINE := 0
endif

ifeq ($(FLASHING_LOCK_WITHOUT_MISC_PART), 1)
...
ABL_FV_IMG: $(EDK_TOOLS_PATH_MARK_FILE)
	...
	-D FORCE_NO_PIE=$(FORCE_NO_PIE) \
	-D VERIFIED_BOOT_ENABLED=$(VERIFIED_BOOT_ENABLED) \
	-D LOAD_KM_AND_SET_ROT=$(LOAD_KM_AND_SET_ROT) \
	-D PVM_SKIP_DTBO=$(PVM_SKIP_DTBO) \
	-D EARLY_ETH_ENABLED=$(EARLY_ETH_ENABLED) \
	-D ENABLE_SAIL_FLASHING=$(ENABLE_SAIL_FLASHING) \
	-D ENABLE_SAIL_BOOT=$(ENABLE_SAIL_BOOT) \
	-D STORE_GPT_ATTR_IN_RECOVERYINFO=$(STORE_GPT_ATTR_IN_RECOVERYINFO) \
	...
	-D SCMI_UPDATES_NEEDED=$(SCMI_UPDATES_NEEDED) \
	-D GET_VIP_BID_INFO=$(GET_VIP_BID_INFO) \
	-D ETH_DT_PATCH_NEEDED=$(ETH_DT_PATCH_NEEDED) \
	-D BOOTIMAGE_LOAD_VERIFY_IN_PARALLEL=$(BOOTIMAGE_LOAD_VERIFY_IN_PARALLEL) \
	-D AB_RETRYCOUNT_DISABLE=$(AB_RETRYCOUNT_DISABLE) \
	-D AB_FORCE_USE_SYSTEM_A=$(AB_FORCE_USE_SYSTEM_A) \
	...
	-D EMMC_MULTI_LUN_SUPPORT=$(EMMC_MULTI_LUN_SUPPORT) \
	-D LOAD_TWO_KM_TAS=$(LOAD_TWO_KM_TAS) \
	-D SDV_DICE_ENABLED=$(SDV_DICE_ENABLED) \
	-D USE_DUMMY_BCC=$(USE_DUMMY_BCC) \
	-D QSPA_BOOTCONFIG_ENABLE=$(QSPA_BOOTCONFIG_ENABLE) \
	-D SOC_OP_MODE_IN_CMDLINE=$(SOC_OP_MODE_IN_CMDLINE) \
	-D FLASHING_LOCK_WITHOUT_MISC_PART=$(FLASHING_LOCK_WITHOUT_MISC_PART) \
