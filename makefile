...
ifndef $(ANDROID_PLATFORM_VERSION)
export ANDROID_PLATFORM_VERSION := 0
endif
...
ifeq ($(TARGET_ARCHITECTURE), arm)
ifneq ($(OVERRIDE_ABL_LOAD_ADDRESS),)
	LOAD_ADDRESS := $(OVERRIDE_ABL_LOAD_ADDRESS)
else
...
ifeq ($(TARGET_NAND_AB_ATTR_SUPPORT), true)
       NAND_AB_ATTR_SUPPORT := 1
else
       NAND_AB_ATTR_SUPPORT := 0
endif

ifeq ($(SUPPORT_DISABLE_NON_BOOTDEVICE), true)
       SUPPORT_DISABLE_NON_BOOTDEVICE := 1
else
       SUPPORT_DISABLE_NON_BOOTDEVICE := 0
endif

ifeq ($(SUPPORT_MODEM_DISABLE), true)
       SUPPORT_MODEM_DISABLE := 1
else
       SUPPORT_MODEM_DISABLE := 0
endif
...
ifneq "$(MTD_UBI_BEB_LIMIT_PER1024)" ""
	MTD_UBI_BEB_LIMIT_PER1024 := $(MTD_UBI_BEB_LIMIT_PER1024)
else
	MTD_UBI_BEB_LIMIT_PER1024 := 0
endif
...
ABL_FV_IMG: EDK_TOOLS_BIN
	...
	-D ANDROID_PLATFORM_VERSION=$(ANDROID_PLATFORM_VERSION) \
	-D NAND_SQUASHFS_SUPPORT=$(NAND_SQUASHFS_SUPPORT) \
	-D MTD_UBI_BEB_LIMIT_PER1024=$(MTD_UBI_BEB_LIMIT_PER1024) \
	-D TARGET_SUPPORTS_EARLY_INIT=$(TARGET_SUPPORTS_EARLY_INIT) \
	...
	-D LINUX_BOOT_CPU_SELECTION_ENABLED=$(LINUX_BOOT_CPU_SELECTION_ENABLED) \
        -D NAND_AB_ATTR_SUPPORT=$(NAND_AB_ATTR_SUPPORT) \
	-D ENABLE_SYSTEMD_BOOTSLOT=$(ENABLE_SYSTEMD_BOOTSLOT) \
	-D SUPPORT_DISABLE_NON_BOOTDEVICE=$(SUPPORT_DISABLE_NON_BOOTDEVICE) \
	-D SUPPORT_MODEM_DISABLE=$(SUPPORT_MODEM_DISABLE) \
