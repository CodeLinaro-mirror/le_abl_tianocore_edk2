...
ifeq ($(BOARD_SUPPORTS_EARLY_INIT),true)
        TARGET_SUPPORTS_EARLY_INIT := TARGET_SUPPORTS_EARLY_INIT=1
else
        TARGET_SUPPORTS_EARLY_INIT := TARGET_SUPPORTS_EARLY_INIT=0
endif

ifeq ($(BOARD_SUPPORTS_EARLY_USB_INIT),true)
	TARGET_SUPPORTS_EARLY_USB_INIT := TARGET_SUPPORTS_EARLY_USB_INIT=1
else
	TARGET_SUPPORTS_EARLY_USB_INIT := TARGET_SUPPORTS_EARLY_USB_INIT=0
endif
...
ifneq ($(TARGET_LINUX_BOOT_CPU_ID),)
	LINUX_BOOT_CPU_ID := LINUX_BOOT_CPU_ID=$(TARGET_LINUX_BOOT_CPU_ID)
else
	LINUX_BOOT_CPU_ID := LINUX_BOOT_CPU_ID=0
endif
...
$(TARGET_ABL): $(LOCAL_ABL_SRC_FILE) | $(ABL_OUT) $(INSTALLED_KEYSTOREIMAGE_TARGET)
	$(MAKEPATH)make -C bootable/bootloader/edk2 \
		...
		$(TARGET_SUPPORTS_EARLY_INIT) \
		$(TARGET_SUPPORTS_EARLY_USB_INIT) \
		...
		BOARD_BOOTLOADER_PRODUCT_NAME=$(BOARD_BOOTLOADER_PRODUCT_NAME) \
	        ANDROID_PLATFORM_VERSION=$(PLATFORM_VERSION)
